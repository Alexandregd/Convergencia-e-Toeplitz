...
\frame{\frametitle{Métodos de Somábilidade}
  Com intuito de generalizar a noção de convergência de sequencias podemos utilizar métodos de somabilidade, que associam uma sequencia com outra possivelmente convergente.
  \begin{block}{Métodos matriciais}
    Um método de somábilidade é nomeado matricial se é possível escreve-lo na forma:
    \begin{equation*}
      y=Ax
    \end{equation*}
...
\frame{\frametitle{Método de Cesàro}
  \begin{block}{Definição}
    O método de Cesàro é a criação de uma nova sequencia utilizando os a media dos n primeiro termos da sequencia e tomando o valor que esta sequencia converge para como o valor que a sequencia original converge.
  \end{block}
  \begin{block}{Representação em somatório}
    Dada uma sequencia $x=(\xi_n)_n$ temos que $y=(\eta_n)_n$ é tal que
    \begin{equation*}
...
\frame{\frametitle{Regularidade e notação}
   Um método matricial pode ser chamado de A-método, se todas as linhas e $y=(\eta_n)_n$ convergem no sentido usual, o limite é denominado A-limite de $x$ e $x$ é dito A-somável, a conjunto de todas as sequencias somáveis é denominado A-domínio.
   \begin{block}{Regularidade}
     Um método matricial é dito regular se toda sequencia convergente pertence ao seu domínio e seu A-limite coincide com o limite usual da sequencia.
   \end{block}
}
...
\frame{\frametitle{Prova}
  \begin{block}{Regularidade$\implies$ Equação (\ref{lim0})}
    Dada a sequencia $x_k$ com todo termo igual a $0$ exceto o termo $k$ que é igual a $1$, temos que $\eta_n=\alpha_{nk}$
  \end{block}
  \begin{block}{Regularidade$\implies$ Equação (\ref{limsum1})}
    Dada a sequencia $x$ com todo termo igual a $1$ temos que
    \begin{equation*}
...
\frame{\frametitle{Prova}
  \begin{block}{Regularidade$\implies$ Equação (\ref{sumgamma})}
    Dado o espaço metrico $c$ com a norma do $\ell^{\infty}$ \\
...
\frame{\frametitle{Prova}
  \begin{block}{Equações (\ref{lim0}), (\ref{limsum1}), (\ref{sumgamma}) $\implies$ Regularidade}
    ...
    que é limitado pois
    \begin{equation*}
      \left\lvert f(x)\right\rvert\leq\left\lVert x\right\rVert
    \end{equation*}
    Dado o conjunto $M$ das sequencias quase constantes, que é denso, e $x\in M$ com todo termo igual apos $j$
  \end{block}
}
  ...
  \begin{block}{Equações (\ref{lim0}), (\ref{limsum1}), (\ref{sumgamma}) $\implies$ Regularidade}
    \begin{align*}
      \eta_n=f_n(x)&=\sum_{k=1}^{j-1}\alpha_{nk}\xi_k+\xi\sum_{k=j}^{\infty}\alpha_{nk} \\
                             ...
                             &\text{Pelas Equações (\ref{lim0}), (\ref{limsum1})} \\
...
\frame{\frametitle{Prova}
  \begin{block}{Equações (\ref{lim0}), (\ref{limsum1}), (\ref{sumgamma}) $\implies$ Regularidade}
    ...
    \end{equation*}
    então $f_n$ é limitado, e como $f_n(x)\to f(x),x\in M$ temos $f_n\overset{w^\ast}{\to}f$ então se $f(x)$ existe $\eta_n\to\xi$ e portanto é regular.
  \end{block}
}
